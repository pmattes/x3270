...
#ifdef XXX_DEBUG
	    fprintf(xxx, "row %d col %d literal '%c' d->sens %p %s current_sens %p %s\n",
		    row, col, *u,
		    (void *)d->sens,
...
#   define N_MATCH 4
	...
	for (y = ul_y; y <= lr_y; y++) {
		...
		fflush(xxx);
#endif
		if (keypad_desc[y][x].sens != NULL && n_matched < N_MATCH) {
		    int i;

		    for (i = 0; i < n_matched; i++)
			if (matches[i] == keypad_desc[y][x].sens)
			    break;
		    if (i >= n_matched)
			matches[n_matched++] = keypad_desc[y][x].sens;
		}
	    }
	}
	if (n_matched) {
		...
		for (i = 0; i < n_matched; i++) {
		    ...
		    overlap[i] = 0;
		    if (yinc) {
			/* Scanning up/down, measure X overlap. */
			for (j = matches[i]->ul_x; j <= matches[i]->lr_x; j++) {
			    if (j >= ul_x && j <= lr_x)
				overlap[i]++;
			    if (j >= ul_x + 1 && j <= lr_x - 1)
				overlap[i]++;
			}
		    } else {
			/* Scanning left/right, measure Y overlap. */
			for (j = matches[i]->ul_y; j <= matches[i]->lr_y; j++) {
			    if (j >= ul_y && j <= lr_y)
				overlap[i]++;
			    if (j >= ul_y + 1 && j <= lr_y - 1)
				overlap[i]++;
			}
		    }
		}
		for (i = 0; i < n_matched; i++)
		    if (best_o < 0 || overlap[i] > overlap[best_o])
			best_o = i;
		for (i = 0; i < n_matched; i++) {
		    if (i != best_o && overlap[i] == overlap[best_o])
			tie++;
		    if (yinc)
			center[i] = find_center_x(matches[i]);
		    else
			center[i] = find_center_y(matches[i]);
		}
#if defined(ZZZ_DEBUG)
		...
		if (tie) {
		    /*
		     * Pick the best-centered match.  That's the one whose
		     * center is closest to the center of the current button.
		     */
		    if (yinc)
			curr_center = find_center_x(current_sens);
		    else
			curr_center = find_center_y(current_sens);
#if defined(ZZZ_DEBUG)
		    fprintf(xxx, "curr_center is %d\n", curr_center);
		    fflush(xxx);
#endif
		    for (i = 0; i < n_matched; i++) {
			if (overlap[i] == overlap[best_o]) {
			    if (best_c < 0 ||
				    abs(curr_center - center[i]) <
				    abs(curr_center - center[best_c])) {
#if defined(ZZZ_DEBUG)
				fprintf(xxx, "center '%s' (%d) is better\n",
					matches[i]->callback,
					center[i]);
				fflush(xxx);
#endif
